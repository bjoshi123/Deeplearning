...
def shuffle_data(x_train, y_train):
    '''
    concate x_train and y_train then shuffle the data and split data into train and test again
    '''
    x_train = x_train.T
    y_train = y_train.T
...
def random_mini_batches(x,y,batch_size=64):
    ...
    mini_batches = []
    no_of_batch = math.floor(data_size/batch_size)

    for i in range(0,no_of_batch):
    ...
    if data_size%batch_size != 0:
        mini_batch_x = x[:, no_of_batch * batch_size:]
        mini_batch_y = y[:, no_of_batch * batch_size:]
        mini_batch = (mini_batch_x, mini_batch_y)
        mini_batches.append(mini_batch)
    return mini_batches
...
def model(mini_batches, layers_dims,layers_activation, learning_rate=0.1, num_iterations=3000, print_cost=False):  # lr was 0.009
    np.random.seed(1)
    costs = []
    parameters = initialize_weight(layers_dims)
    mini_batche_size = len(mini_batches)
    for i in range(0, num_iterations):
        for j in range(mini_batche_size):
            X,Y = mini_batches[j]
...
#shuffle data before mini batch
x_train, y_train = shuffle_data(x_train, y_train)

#get mini batches
mini_batches = random_mini_batches(x_train,y_train,batch_size=16)

#train model
parameters = model(mini_batches, layer_dims,learning_rate=0.0075,num_iterations=3000, print_cost=True, layers_activation=layer_activation)
